...
const Checkout = ({ data, success, meta }) => {
  ...
  const mapIdsToServices = () => {
    return data.service.id.map((serviceId) =>
  ...
  return (
    <div className={classes.Checkout}>
      <div className={classes.Main}>
        ...
        <div className={classes.Schedule}>
          <p className={classes.Time}>
            {("0" + new Date(data.service.time).getHours()).slice(-2) +
              ":" +
              ("0" + new Date(data.service.time).getMinutes()).slice(-2)}
          </p>
          <div className={classes.Separation} />
          <p className={classes.Date}>
            {new Date(data.service.time).getDate() +
              " " +
              monthNames[new Date(data.service.time).getMonth()]}
            <br />
          ...
          {mapIdsToServices().map((service, i) => (
            <div className={classes.Service} key={i}>
              ...
              <p className={classes.Duration}>
                {service.duration >= 60 && ~~(service.duration / 60) + " hrs"}{" "}
        ...
        <div
          className={[
            classes.Box,
            data.details.payment === "card" ? classes.Card : "",
          ].join(" ")}
        >
      ...
      <div className={classes.Submit}>
        {data.details.payment === "card" && (
          <p className={classes.Terms}>
            By clicking &apos;Pay now&apos; button, you agree to our{" "}
            <span>Booking Terms</span>
          </p>
        )}
        <button className={classes.Button} onClick={onSubmitHandler}>
          {data.details.payment === "venue" ? "Buy now" : "Pay now"}
        </button>
      </div>
    </div>
  );
