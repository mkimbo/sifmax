...
export const Carousel = ({ items, initialScroll = 0 }: CarouselProps) => {
  ...
  const handleCardClose = (index: number) => {
    if (carouselRef.current) {
      const cardWidth = isMobile() ? 230 : 320; // (md:w-96)
  ...
  return (
    <>
      <AnimatePresence>
        {open && (
          <div className="fixed inset-0 h-screen z-50 overflow-auto">
            ...
            >
              <button
                className="sticky top-16 md:top-20 h-8 w-8 right-0 ml-auto bg-black dark:bg-white rounded-full flex items-center justify-center"
                onClick={handleClose}
              >
              ...
              <motion.p
                ...
                className="text-xl md:text-3xl font-semibold  satisfy text-primary "
              >
                {card.category}
              </motion.p>
              <motion.p
      ...
      <motion.button
        layoutId={layout ? `card-${card.title}` : undefined}
        onClick={handleOpen}
        className="rounded-3xl bg-gray-100 dark:bg-neutral-900 h-80 w-56 md:h-[32rem] md:w-80 overflow-hidden flex flex-col items-start justify-start relative z-10"
      >
        <div className="absolute h-full top-0 inset-x-0 bg-black/50 z-30 pointer-events-none" />
        <div className="relative z-40 p-8">
          <motion.p
            layoutId={layout ? `category-${card.category}` : undefined}
            className="text-white text-xl md:text-3xl font-semibold  satisfy text-left "
          >
            {card.category}
          </motion.p>
          <motion.p
          ...
          <motion.p
            ...
            className="flex flex-row gap-1 items-center align-middle text-primary text-sm md:text-base font-medium  max-w-xs text-left  mt-2"
          >
            Service list
...
export function ServiceCardsDemo() {
...
const DummyContent = ({ services }: Props) => {
  return (
    <>
      {Object.entries(
        ...
        <div
          key={category}
          className="w-full bg-[#F5F5F7] p-8 md:p-14 rounded-3xl mb-4"
        >
          {array.length > 1 && (
            <h4 className="font-medium text-lg text-center">{category}</h4>
          )}
          {services.map((service: { title: string; price: string }) => (
            <ServiceCard
...
const services = sifmaxServiceTypes.map((service) => {
  return {
    ...
    src: service.icon,
    content: <DummyContent services={getServices(service.services)} />,
  };
});
